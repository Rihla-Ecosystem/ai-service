...
## Changelog
- 2026-08-19: PULLED origin/main →ba7d82f (`fix: normalize Jina embedding token usage`) and DEPLOYED on VPS — rihla-ai recreated from ghcr `ai-service:latest` (image 2026-08-17T20:44Z), healthy. /health OK.
- 2026-08-14: ZONE-LAW RAG ENDPOINT SHIPPED. Committed+pushed `f68a962` — new `app/api/legal.py` (zone-law endpoint, registered in `main.py`) + CONTEXT checkpoint. CI rebuilt GHCR image (09:01Z), deployed on VPS (`rihla-ai` healthy on new image). Rebased onto org main which had 5 AI-billing commits ahead; rebase clean (only legal.py + main.py touched).
- 2026-08-09: Demo AI hardening: Core stream now forwards the bounded existing history shape and aborts/cleans up an upstream stream after 120 seconds, marking dispatch failures indeterminate through the existing stream recovery path. ai-service accepts bounded user/assistant history, caps live chat/tool/image/audio text output at 1200, and clamps Gemini retries to 0..2 (at most three attempts). TTS remains uncapped pending provider/SDK semantic verification.
...
- 2026-08-14: FIXED `app/api/legal.py` RAG parsing + E2E verified. Was looking for `payload.data` (never present) → rules always empty. Now parses chunk `text` + `metadata.section`; `sections` chunks are Python-`repr` (single quotes) so parser tries `json.loads` then falls back to `ast.literal_eval`; citations come from `metadata.source_file`. Verified live: `/legal?class_name=restricted` → rules ["Title","Drone Regulations (Law No. 216 of 2017)","Public Photography Regulations…"] + citations to 01-06 legal files. Core forwards `class` (not `class_name`) — matches Core→ai wire contract. RAG legal collection (`rihla_legal`) populated from `data/rag/Legal_Frameworks_and_Culture_Regulations/01_*..06_*.json`; one chunk per top-level JSON key.
